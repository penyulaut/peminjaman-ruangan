                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" aria-label="Default select example" name="room_id"
                            ...
                            @foreach ($rooms as $room)
                                @if ($room->kode == request()->segment(count(request()->segments())))
                                    <option value="{{ $room->id }}" selected>{{ $room->kode }} - {{ $room->name }}</option>
                                @else
                                    <option value="{{ $room->id }}">{{ $room->kode }} - {{ $room->name }}</option>
                                @endif
                            @endforeach
                        </select>
                    </div>
